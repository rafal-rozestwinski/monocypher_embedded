...
#define TEXT_SIZE 70
    ...
    crypto_lock_aead(mac, cipher_text, key, nonce, ad, ad_size, text_to_encrypt, strlen(text_to_encrypt));
    ...
    uint8_t ad2[] = "ojej plaiN extra data";
    ...
    int ret = crypto_unlock_aead(decrypted, key, nonce, mac, ad, ad_size, cipher_text, strlen(text_to_encrypt));
    char* decrypted_ch = (char*)decrypted;
    decrypted_ch[strlen(text_to_encrypt)] = '\0';
    printf("unlock ret = %i\n", ret);
    printf("decrypted = '%s'\n", decrypted_ch);
}
